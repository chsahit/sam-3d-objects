...
from sam3d_objects.pipeline.inference_pipeline_sequential import InferencePipelineSequential
...
class InferencePipelinePointMapSequential(InferencePipelineSequential):
    ...
    def compute_pointmap(self, image, pointmap=None):
        ...
        if pointmap is None:
            ...
            if self.depth_model is not None:
                self.depth_model.model.cpu()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                logger.info("CUDA cache cleared after depth model.")
        else:
            output = {}
