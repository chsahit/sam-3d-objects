...
class InferencePipelineSequential:
    def __init__(
        self,
        ...
        logger.info("Sequential mode: Storing model paths instead of loading all models...")

        # Store model configurations and paths for lazy loading
        self.model_configs = {
            "ss_generator": {
                "config_path": ss_generator_config_path,
                "ckpt_path": ss_generator_ckpt_path,
            },
            "slat_generator": {
                "config_path": slat_generator_config_path,
                "ckpt_path": slat_generator_ckpt_path,
            },
            "ss_decoder": {
                "config_path": ss_decoder_config_path,
                "ckpt_path": ss_decoder_ckpt_path,
            },
            "ss_encoder": {
                "config_path": ss_encoder_config_path,
                "ckpt_path": ss_encoder_ckpt_path,
            },
            "slat_decoder_gs": {
                "config_path": slat_decoder_gs_config_path,
                "ckpt_path": slat_decoder_gs_ckpt_path,
            },
            "slat_decoder_gs_4": {
                "config_path": slat_decoder_gs_4_config_path,
                "ckpt_path": slat_decoder_gs_4_ckpt_path,
            },
            "slat_decoder_mesh": {
                "config_path": slat_decoder_mesh_config_path,
                "ckpt_path": slat_decoder_mesh_ckpt_path,
            },
        ...
        # Store loaded models temporarily
        self.models = {}
        self.condition_embedders = {}

        # Store CFG overrides for generators
        self.ss_cfg_overrides = {
        ...
        logger.info("Model paths stored. Models will be loaded on-demand.")

        if self.compile_model:
            logger.warning("Model compilation is not supported in sequential mode. Ignoring compile_model flag.")
    ...
    def _load_model(self, model_name):
        ...
        if model_name in self.condition_embedders:
            logger.info(f"Condition embedder {model_name} already loaded, skipping...")
            return self.condition_embedders[model_name]
        ...
        with self.device:
            if model_name == "ss_generator":
                model = self.init_ss_generator(config["config_path"], config["ckpt_path"])
                self.override_ss_generator_cfg_config(model, **self.ss_cfg_overrides)
                self.models[model_name] = model
            elif model_name == "slat_generator":
                model = self.init_slat_generator(config["config_path"], config["ckpt_path"])
                self.override_slat_generator_cfg_config(model, **self.slat_cfg_overrides)
                self.models[model_name] = model
            elif model_name == "ss_decoder":
                model = self.init_ss_decoder(config["config_path"], config["ckpt_path"])
                self.models[model_name] = model
            elif model_name == "ss_encoder":
                model = self.init_ss_encoder(config["config_path"], config["ckpt_path"])
                self.models[model_name] = model
            elif model_name == "slat_decoder_gs":
                model = self.init_slat_decoder_gs(config["config_path"], config["ckpt_path"])
                self.models[model_name] = model
            elif model_name == "slat_decoder_gs_4":
                model = self.init_slat_decoder_gs(config["config_path"], config["ckpt_path"])
                self.models[model_name] = model
            elif model_name == "slat_decoder_mesh":
                model = self.init_slat_decoder_mesh(config["config_path"], config["ckpt_path"])
                self.models[model_name] = model
            elif model_name == "ss_condition_embedder":
                model = self.init_ss_condition_embedder(config["config_path"], config["ckpt_path"])
                self.condition_embedders[model_name] = model
            elif model_name == "slat_condition_embedder":
                model = self.init_slat_condition_embedder(config["config_path"], config["ckpt_path"])
                self.condition_embedders[model_name] = model
            else:
    ...
    def _unload_models(self, *model_names):
        """Unload specific models from memory and clear CUDA cache."""
        for model_name in model_names:
            if model_name in self.models:
                logger.info(f"Unloading model: {model_name}...")
                del self.models[model_name]
            if model_name in self.condition_embedders:
                logger.info(f"Unloading condition embedder: {model_name}...")
                del self.condition_embedders[model_name]
    ...
    def decode_slat(
        ...
        models_to_load = []
        if "mesh" in formats:
            models_to_load.append("slat_decoder_mesh")
        if "gaussian" in formats:
            models_to_load.append("slat_decoder_gs")
        if "gaussian_4" in formats:
            models_to_load.append("slat_decoder_gs_4")
        ...
        # Unload decoder models
        self._unload_models(*models_to_load)

        return ret

    def is_mm_dit(self, model_name="ss_generator"):
        # Load the model temporarily to check
    ...
    def sample_sparse_structure(
        ...
        # Load SS models
        logger.info("Loading sparse structure models...")
        self._load_model("ss_generator")
        self._load_model("ss_decoder")
        self._load_model("ss_condition_embedder")
    ...
    def sample_slat(
        ...
        # Load SLAT models
        logger.info("Loading SLAT models...")
        self._load_model("slat_generator")
        self._load_model("slat_condition_embedder")
